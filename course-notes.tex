...
In the case of $n$ features, each feature $x^{(i)}$ is a $n+1$ dimensional feature vector with $x_0$ set to $1$ for convenience:
\begin{equation*}
  x^{(i)}=\begin{pmatrix}x^{(i)}_0\\x^{(i)}_1\\\vdots\\x^{(i)}_n\end{pmatrix}\in\mathbb{R}^{n+1}
  \mathrm{\ where\ }x^{(i)}_0\coloneqq 1
\end{equation*}
The hypothesis $h$ is
\begin{equation*}
