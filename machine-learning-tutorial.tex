...
These are course notes to Andrew Ng's video lecture\citep{andrewng,andrewng2}\footnote{also available here: \url{https://www.youtube.com/playlist?list=PL0Smm0jPm9WcCsYvbhPCdizqNKps69W4Z}}.
...
\end{itemize}
where the definition of the sigmoid function $g$ is extended to work element-wise on vectors and prepend $1$ as the first element.
...
\begin{itemize}
  ...
  \item $\big(h_\theta(x)\big)_i$ is the $i$th component of the vector $h_\theta(x)$
\end{itemize}
...
\begin{itemize}
  \item collect lots of data (\emph{e.g.} ``honeypot'' project)
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{Cost terms for $y=0$ and $y=1$ with $\theta=1$\label{fig:logcost}}
  \end{center}
\end{figure}
...
Note that the data is highly skewed (y=1 is rare), it is necessary to use a good evaluation metric (see Section\ref{cha:f1}).
...
For example one can attempt to predict movie ratings for new movies not rated by a user yet.
...
\begin{figure}[htbp]
  ...
  \end{center}
\end{figure}
\emph{I.e.} a classifier is trained to detect character boundaries.
...
The \ac{LSTM} is defined as follows\citep{wiki:lstm,karpathy2016cs231n}:
\begin{align*}
  \begin{split}
    f_t &= \sigma(W_{f} x_t + U_{f} h_{t-1} + b_f) \\
    i_t &= \sigma(W_{i} x_t + U_{i} h_{t-1} + b_i) \\
    o_t &= \sigma(W_{o} x_t + U_{o} h_{t-1} + b_o) \\
    g_t &= \operatorname{tanh}(W_{c} x_t + U_{c} h_{t-1} + b_c) \\
...
\begin{itemize}
  ...
  \item $h_t$ is the (hidden or output) layer at time $t$
...
\end{equation*}
See Appendix \ref{app:lstm} for an implementation.
...
\ac{RNN} are training using \ac{BPTT} or truncated \ac{BPTT}.
...
Training the \ac{RNN} using some of William Shakespeare's work\citep{shakespeare}, the predictor generates random output like this:
\begin{footnotesize}
\begin{verbatim}
                     88
  If then growt my grown,
  Which perfect should checcious arting they song more some shalons sure mads, your near thou moul they seen,
  But in the fair, nate,
  Or my see eyes we tomb,
  The be as the largant a prays in dead me.
\end{verbatim}
\end{footnotesize}
...
\subsubsection{Long Short-Term Memory}\label{app:lstm}
The following example is a Tensorflow implementation of a \ac{LSTM} with a fully connected output layer for predicting the next character in a text.
The implementation is inspired by Hannah Vivian Shaw's work\footnote{\url{https://github.com/vivshaw/shakespeare-LSTM/}}.
Training the \ac{LSTM} using some of William Shakespeare's work\citep{shakespeare}, the predictor generates random output like this:
\begin{footnotesize}
\begin{verbatim}
     To beward therefore woet, and some and I his againge
  Then I blamentles which girn, make afteek,
  Apther these had in me it as me.
  If their world up by me thee thought love,
  And the pairhed bein that how thee,
  Some of my self in strance dorchuard engress
  Some in memink's shalt can a obell grow them bear
  Make picture) in these the heart gone,
  And their show the can strall doth hand.
\end{verbatim}
\end{footnotesize}

\paragraph{Training}
\inputminted[frame=lines,linenos,fontsize=\small]{python}{lstm.py}

\paragraph{Predicting}
\inputminted[frame=lines,linenos,fontsize=\small]{python}{lstm-predict.py}
...
\begin{itemize}
  ...
  \item \ac{ReLU}
  \item BatchNorm\footnote{\url{https://wiseodd.github.io/techblog/2016/07/04/batchnorm/}}
  \item Dropout\citep{srivastava14a}
  \item ADAM\citep{kingma2014adam}
  \item Increasing the Batch size\citep{smith_dont_2017}
  \item Early stopping
  \item Graycode
  \item Dual-Tree Complex Wavelets
\end{itemize}

\subsubsection{See also}
\begin{itemize}
  \item DeepVO: Towards End-to-End Visual Odometry with Deep Recurrent Convolutional Neural Networks\citep{wang2017deepvo}
  \item Mask R-CNN for Object Detection and Segmentation\footnote{\url{https://github.com/matterport/Mask_RCNN}}
\end{itemize}
...
\begin{acronym}[L-BFGS]
  \acro{BPTT}{Backward Propagation Through Time}
  ...
  \acro{LSE}{Least Squares Estimator}
  \acro{LSI}{Local Shift Invariant}
