...
For large scale machine learning problems it might be necessary to distribute the computation over several machines.
    ...
    \begin{itemize}
      \item machine 1
        \begin{itemize}
          ...
          \item compute $temp^{(1)}_j=\sum_{i=1}^{100}\big(h_\theta(\sxi)-\syi)$
        \end{itemize}
      \item machine 2
        \begin{itemize}
          \item use $(x^{(101)},y^{(101)}),\ldots,(x^{(200)},y^{(200)})$
          \item compute $temp^{(2)}_j=\sum_{i=101}^{200}\big(h_\theta(\sxi)-\syi)$
        \end{itemize}
      \item $\vdots$
    \end{itemize}
