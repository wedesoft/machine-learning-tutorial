...
One can define new features using the proximity to landmarks (\emph{e.g.} $l^{(1)},l^{(2)},l^{(3)}$ and assuming $\theta_0=0$ for simplification).
\begin{align*}
  \begin{split}
    f_1=similarity(x,l^{(1)})&=\operatorname{e}^{-\cfrac{||x-l^{(1)}||^2}{2\sigma^2}}=\operatorname{e}^{-\cfrac{\sum_{j=1}^n(x_j-l^{(1)}_j)^2}{2\sigma^2}}\\
    f_1=similarity(x,l^{(2)})&=\operatorname{e}^{-\cfrac{||x-l^{(2)}||^2}{2\sigma^2}}\\
    f_1=\underbrace{similarity}_{\mathrm{kernel}}(x,l^{(3)})&=\underbrace{\operatorname{e}^{-\cfrac{||x-l^{(3)}||^2}{2\sigma^2}}}_{\mathrm{Gaussian\ kernel}}
  \end{split}
\end{align*}
\emph{I.e.}
\begin{itemize}
