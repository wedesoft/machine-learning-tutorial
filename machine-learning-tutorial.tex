...
\begin{equation*}
  \mathop{\operatorname{min}}_{\theta^{(j)}}\frac{1}{2\,\cancel{m^{(j)}}}\sum_{i;r(i,j)=1}\big((\theta^{(j)})^\top(\sxi)-y^{(i,j)}\big)^2+\frac{\lambda}{2\,\cancel{m^{(j)}}}\sum_{k=1}^n(\theta^{(j)}_k)^2
\end{equation*}
The optimization objective for all users $\theta^{(1)},\theta^{(2)},\ldots,\theta^{(n_u)}$ is:
\begin{equation*}
  \mathop{\operatorname{min}}_{\theta^{(1)},\ldots,\theta^{(n_u)}}\frac{1}{2}\sum_{j=1}^{n_u}\sum_{i;r(i,j)=1}\big((\theta^{(j)})^\top(\sxi)-y^{(i,j)}\big)^2+\frac{\lambda}{2}\sum_{j=1}^{n_u}\sum_{k=1}^n(\theta^{(j)}_k)^2
\end{equation*}
...
\end{itemize}
One can use this two algorithms starting with a random guess for $\theta$ to estimate $x$ and then use $x$ to get an improved estimate for $\theta$ ...
