...
One can define new features using the proximity to landmarks (\emph{e.g.} $l^{(1)},l^{(2)},l^{(3)}$).
\begin{align*}
  \begin{split}
    f_1=similarity(x,l^{(1)})&=\ \ \,\operatorname{e}^{-\frac{||x-l^{(1)}||^2}{2\sigma^2}}\\
    f_1=similarity(x,l^{(2)})&=\ \ \,\operatorname{e}^{-\frac{||x-l^{(2)}||^2}{2\sigma^2}}\\
    f_1=\underbrace{similarity}_{\mathrm{kernel}}(x,l^{(3)})&=\underbrace{\operatorname{e}^{-\frac{||x-l^{(3)}||^2}{2\sigma^2}}}_{\mathrm{Gaussian\ kernel}}
  \end{split}
\end{align*}
